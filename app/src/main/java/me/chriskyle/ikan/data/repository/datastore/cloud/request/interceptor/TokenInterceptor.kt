...
import me.chriskyle.ikan.data.repository.datastore.cloud.service.TokenService
...
class TokenInterceptor @Inject internal constructor(private val bus: Bus, connector: Connector,
    ...
    fun <T> refreshTokenAndRetry(toBeResumed: Observable<T>): Function<Throwable, out Observable<out T>> {
        return Function { topThrowable ->
            if (isHttp401Error(topThrowable)) {
                refreshToken()
                        .doOnError { throwable ->
                            if (isHttp403Error(throwable)) {
                                bus.post(ReSignInEvent())
                            }
                        }
                        .doOnNext { tokenEntity -> tokenCache.put(tokenEntity) }
                        .flatMap { toBeResumed }
            } else Observable.error(topThrowable)
        }
    }

    private fun refreshToken(): Observable<TokenEntity> {
        val tokenParamProvider = TokenParamProvider()
        tokenParamProvider.reToken(tokenCache.get()?.reToken!!)
